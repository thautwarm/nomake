...
export class Compilation
{
    ...
    async dumpCmake(options: CMakeProjectOptions)
    {
        ...
        const sourceFilesAgg = this.sources.map(x => aggFile(
            x,
            TAG_SOURCE_FILE
        ));

        const includeDirsAgg = this.includeDirs.map(x => aggDir(
            x,
            'copy',
            TAG_INCLUDE_DIR
        ));
        const libraryDirsAgg = this.libraryDirs.map(x => aggDir(
            x,
            'copy',
            TAG_LIBRARY_DIR
        ));
        const aggregated = aggregatePath([...sourceFilesAgg, ...includeDirsAgg, ...libraryDirsAgg]);
        ...
        for (let i = 0; i < aggregated.length; i++)
        {
            ...
            {
                throw new Error(`Unexpected file processing in make project: ${aggregated[i].path}`)
            }
            else
            {
...
export abstract class CCompiler
{
    ...
    abstract compileSharedLib(compilation: Compilation, dest: string): Promise<void>
...
export class GCC extends CCompiler
{
    ...
    async nativeCompile(
        compilation: Compilation, dest: string,
        mode: 'shared' | 'exe'
    )
    {
        const argv: string[] = []
...
export type AggregatedPath =
    ...
    | { path: string, kind: 'dir', content: Map<string, AggregatedPath> | 'copy', tag: number }
...
function aggDir(path: string, content: Map<string, AggregatedPath> | 'copy', tag?: number): AggregatedPath
{
...
export class Zig extends CCompiler
{
    ...
    )
    {
        ...
        if (this.zigToolchain.os == 'windows')
        {
            ...
            const source = destPath.parent.join(new NM.Path(firstSourceFile).withExt(".lib").name);
            try
            {
                if (!source.isFile())
                {
